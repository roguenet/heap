...
const PROPS = {
  config: {
    local: ['name', 'copyright', 'copyrightCovers', 'remote', 'cdnPrefix'],
 ...
 * @param cdnPrefix The prefix to apply to the final image URL
 * @returns Promise that resolves to the sources array for the resulting images.
 */
async function exportImage (targetDir, card, args, imagePath, cdnPrefix) {
  ...
  for (let sizeStr of sizes.split(',')) {
    ...
    promises.push(resize(size, imagePath, toPath, width, height).then(image => ({
      ...image,
 ...
 *
 * @param localConfig Heap config
 * @param card The card to export
 * @param targetDir The target directory
 * @param args The command args
 * @returns Promise that resolves to the complete card definition.
 */
async function exportCard (localConfig, card, targetDir, args) {
  const { config: configPath, jsonOnly } = args
  const { cdnPrefix } = localConfig
  ...
  switch (card.cardType) {
    case 'photo': {
      ...
      const imagePath = path.resolve(path.dirname(configPath), card.filePath)
      const sources = await exportImage(targetDir, card, args, imagePath, cdnPrefix)

      const { filePath, width, height } = card
      const filename = path.resolve(path.dirname(configPath), filePath)
    ...
    case 'title': {
      ...
      if (descriptionFile != null) {
        ...
        const filePath = path.resolve(path.dirname(configPath), descriptionFile)
...
export function logConfig (config, configPath) {
  Log.feedback('\nHeap Config Path:', path.resolve(configPath))
  Log.feedback(
    'Remote configuration',
    config.remote == null ? Log.red('undefined') : JSON.stringify(config.remote)
  )
  Log.feedback('CDN Prefix', config.cdnPrefix == null ? Log.red('undefined') : config.cdnPrefix)
  Log.feedback('Cards', config.cards.length)
  Log.feedback('Photos', photos(config).length)
}
